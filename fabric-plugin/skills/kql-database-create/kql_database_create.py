...
def create_kql_database(workspace_id, name, description=None, eventhouse_id=None):
    """Create a new KQL database."""
    check_security(workspace_id, "kql:create")
    ...
    if eventhouse_id:
        body['creationPayload'] = {'databaseType': 'ReadWrite',
    ...
    try:
        ...
        if resp.status in (200, 201):
            ...
            print(f"\n[SUCCESS] KQL database created:")
            print(f"  Name: {result.get('displayName', name)}")
            print(f"  ID:   {result.get('id', 'N/A')}")
            return 0
        elif resp.status == 202:
            location = resp.headers.get('Location', '')
            print("[INFO] Creating KQL database...")
            if location:
                for i in range(20):
                    time.sleep(5)
                    try:
        ...
        if e.code == 409:
            print("[ERROR] A KQL database with this name already exists.")
            return 1
...
def main():
    cli = SkillCLI('kql_database_create.py',
                   'Create a new KQL database in a workspace')
    cli.workspace()
    cli.positional('name', help='KQL database display name')
    cli.positional('description', nargs='?', default=None,
                   help='Optional description')
    cli.opt('--eventhouse-id', dest='eventhouse_id',
            help='Parent eventhouse item ID (GUID)')
    args = cli.parse()

    sys.exit(create_kql_database(args.workspace_id, args.name,
                                 args.description, args.eventhouse_id))
